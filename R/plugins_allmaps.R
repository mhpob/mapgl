...
#' @param id A unique ID for the warped map layer. If NULL, it will default to
#'  "warped-map-layer". Each georeferenced map must have a unique ID.
...
### Shiny tools
#' @export
set_georeferenced_map_opacity <- function(
  map,
  layer_id,
  opacity
) {
  if (any(inherits(map, "mapboxgl_proxy"), inherits(map, "maplibre_proxy"))) {
    if (
      ...
      map$session$sendCustomMessage(
        ...
        list(
          id = map$id,
          message = list(
            type = "set_georeferenced_map_opacity",
            id = layer_id,
            opacity = opacity,
            map = map$map_side
          )
    ...
    if (is.null(map$x$setOpacity)) {
      map$x$setOpacity <- list()
    }
    map$x$setOpacity[[length(map$x$setOpacity) + 1]] <- list(
      id = layer_id,
      opacity = opacity
    )
  }
...
set_georeferenced_map_color <- function(
  map,
  layer_id,
  color
) {}

set_georeferenced_map_saturation <- function(
  map,
  layer_id,
  saturation
) {}

set_georeferenced_map_remove_color <- function(
  map,
  layer_id,
  color = NULL,
  threshold = NULL,
