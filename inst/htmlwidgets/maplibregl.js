...
if (HTMLWidgets.shinyMode) {
  Shiny.addCustomMessageHandler("maplibre-proxy", function (data) {
    ...
    if (map) {
      ...
      } else if (message.type === "add_georeferenced_map") {
        allmaps(map, message.georeferenced_map);
      } else if (message.type === "set_georeferenced_map_opacity") {
        warpedMapLayers[0].setOpacity(
          message.opacity
        )
